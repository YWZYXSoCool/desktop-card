...
export interface WidgetRuntime {
    ...
    /** 处理被拖入的文件（可能一次多份）：widget 自行判定是否处理，不处理则静默忽略 */
...
/** 裸 HTTP 原始响应：只给状态码 + 正文原文，**不解析**。业务解析交给 widget。 */
...
/**
 * 通用网络/文件下载原语（仅申请了 `download` 权限才有）。
 * 全部同步返回；后端不关心业务，业务（API 构造、解析、选流、拼文件名）由 widget 负责。
 * 下载任务进度变更会触发宿主 `widget-progress` 事件 → 沙箱 widget 自动重渲染。
 */
export interface SandboxDownload {
    /** 裸 HTTP：转发 method/url/headers/body，返回原始状态码 + 正文（适合 JSON/XML 解析类请求）。 */
    http(
        method: string,
        url: string,
    ...
    /** 把 URL 流式下载到下载目录的指定文件名，返回 job id。 */
    download(url: string, headers: Record<string, string>, filename: string): number;
    /** 把文本写入下载目录的指定文件名，返回 job id。 */
    writeText(filename: string, content: string): number;
    /** 当前下载目录路径。 */
    dir(): string;
    /** 全部任务状态快照。 */
...
export interface SandboxWidgetRuntime {
    ...
    /** 处理被拖入的文件（可能一次多份）：widget 自行判定是否处理，不处理则静默忽略。 */
    onDrop?(paths: string[], ctx: SandboxContext): void;
}
