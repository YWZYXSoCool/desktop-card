...
| 功能 | 说明 |
|------|------|
| 拖入即播 | 拖拽单个音频文件（mp3 / wav / ogg / m4a / aac / flac）到卡片，松开即播 |
| 播放 / 暂停 | 点击播放按钮或左侧封面切换 |
| 循环播放 | 进度条右侧循环按钮切换：开=播完从头重播，关=播完暂停；重启后恢复 |
| 进度显示 + 拖拽定位 | 进度条可点击 / 拖动跳转 |
| 音量控制 | 音量滑块 + 静音开关，重启后恢复 |
| 曲名 / 时长 | 曲名单行截断（hover 显示完整路径），时长 `m:ss / m:ss` |
| 专辑封面 | 音频含嵌入封面时显示在左侧封面位，无则回落占位音符图标 |
| 恢复上次文件 | 记住上次播放的文件，重启后恢复（加载但不自动播放） |
...
| Ctrl+S widget 设置 | 聚焦时按 Ctrl+S 弹出**居中的独立窗口**，平台统一渲染当前 widget 的声明式设置项（无则提示），改动经事件同步并持久化 |
...
external-widgets/              # 开发期外部 widget 示例（counter；生产放 %APPDATA%/…/widgets）
...
| `playback.loop` | false | 循环播放（播完从头重播） |
| `player.lastFile` | "" | 上次播放的文件路径，重启后恢复（加载但不自动播放） |
